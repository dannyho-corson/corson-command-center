...
function ExpandedRow({ prospect, colSpan, onUpdated, onDeleted, onClose }) {
  ...
  return (
    <tr className="bg-gray-900/50">
      <td colSpan={colSpan} className="px-4 py-4 border-b border-gray-800">
...
// ── PROSPECTS TABLE (shared component) ───────────────────────────────────────
// Used by /ar-inbox (full mode) and the Hip Pocket section on /artists (compact).
//
// Props:
//   compact   — boolean. When true, hides the "Decide by" column to match the
//               5-column embedded variant (Name | Type | Status | Source | Notes).
//               Default false (full 6-column view for /ar-inbox).
//
// Owns its own data fetch, add modal, status changes, inline edit, and filter
// pills. Page-level chrome (Nav, page header) lives on the consuming page.
export default function ProspectsTable({ compact = false }) {
  ...
  // Column count drives ExpandedRow's colSpan and the "compact" trim
  const showDecideBy = !compact;
  const colSpan = showDecideBy ? 6 : 5;
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-[11px] uppercase tracking-wider text-gray-500 border-b border-gray-800">
                ...
                {showDecideBy && <th className="px-4 py-2.5 font-semibold">Decide by</th>}
